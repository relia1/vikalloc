...
// changed the default to NEXT_FIT
vikalloc_fit_algorithm_t algo = NEXT_FIT;
...
{
    fprintf(log_stream, "vikalloc first fit tests not implemented\n");
    exit(EXIT_FAILURE);
}
...
best_fit_tests(void)
{
    fprintf(log_stream, "vikalloc best fit tests not implemented\n");
    exit(EXIT_FAILURE);
}
...
{
    fprintf(log_stream, "vikalloc worst fit tests not implekemted\n");
    exit(EXIT_FAILURE);
}
...
basic1(int testno)
{
    ...
    vikalloc_reset();
    ptr1 = sbrk(0);
    fprintf(log_stream, "  ptr : " PTR_N, (ptr1 - base));
    assert(ptr1 == base);
